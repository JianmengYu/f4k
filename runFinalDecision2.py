    ...
    def run(self):
        ...
        while not self.work_queue.done():
            ...
            for slave_return_data in self.work_queue.get_completed_work():
                done, message, movidf, f, fe, evr, nf = slave_return_data
                FISH += f
                FEIF += fe
                EVR += evr
                NONFISH += nf
...
def do_actual_work(self, data, q):
    ...
    try:
        ...
        q.put([True, 'Slave: %s skipped   %s because it\'s empty' % (name, idee), movid,0,0,0,0])
        return
    
    # ==================== PROCESSING ====================
     
    feif = np.sum(result[:,:,6],1) > 3.999
    evr = np.sum(result[:,:,1],1) > 3.999
    ...
    q.put([True, retString, movid, f, fe, evr, nf])
    return
