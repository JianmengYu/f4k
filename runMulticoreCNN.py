...
def do_actual_work(self, data, q):
    ...
    savepath = "/afs/inf.ed.ac.uk/group/ug4-projects/s1413557/CNNoutput/{0}/{1}/{2}".format(idee[0],idee[0:2],idee)
    ...
    yhat = np.zeros((frames,3,10))

    #cv2.CV_IMWRITE_JPEG_QUALITY = 100
    
    #try:
    #    os.makedirs("/afs/inf.ed.ac.uk/group/project/F4KC/temp/" + fname + "/")
    #except Exception:
    #    print("waa")
    ...
    for f in range(frames):
        ...
        #Image_N
        if cvtInPython:
            image_N = cv2.cvtColor(image_N, cv2.COLOR_RGB2YUV).astype(np.float)/255.0
            xt = torch.fromNumpyArray(np.transpose(image_N,(2,0,1)))._float()
        else:
            xt = torch.fromNumpyArray(np.transpose(image_N,(2,0,1)).astype(np.float)/255.0)._float()
            xt = image.rgb2yuv(xt)
        for i in range(3):
            xt[i]._add(-meann[1][i])
            xt[i]._div(stdd[1][i])
        ...
        #Image_W_C
        if cvtInPython:
            image_WC = cv2.cvtColor(image_WC, cv2.COLOR_RGB2YUV).astype(np.float)/255.0
            xt = torch.fromNumpyArray(np.transpose(image_WC,(2,0,1)))._float()
        else:
            xt = torch.fromNumpyArray(np.transpose(image_WC,(2,0,1)).astype(np.float)/255.0)._float()
            xt = image.rgb2yuv(xt)
        for i in range(3):
            xt[i]._add(-meann[2][i])
            xt[i]._div(stdd[2][i])
        ...
        #Image_B_C
        if cvtInPython:
            image_BC = cv2.cvtColor(image_BC, cv2.COLOR_RGB2YUV).astype(np.float)/255.0
            xt = torch.fromNumpyArray(np.transpose(image_BC,(2,0,1)))._float()
        else:
            xt = torch.fromNumpyArray(np.transpose(image_BC,(2,0,1)).astype(np.float)/255.0)._float()
            xt = image.rgb2yuv(xt)
        for i in range(3):
            xt[i]._add(-meann[3][i])
            xt[i]._div(stdd[3][i])
