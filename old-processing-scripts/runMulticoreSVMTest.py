...
#ls2 = np.linspace(20,20,cs)

pcs = 15
ls3 = np.linspace(30,100,pcs)
...
class MySlave(Slave):
    ...
    def __init__(self):
        ...
        self.features, self.targets = loadTrainDataSet(includeNew=True)
        self.X = self.features[:,:]
        self.y = self.targets[:].astype('int') 
        self.features2, self.targets2 = loadValidationSet(split=1)
        self.X2 = self.features2[:,:]
        self.y2 = self.targets2[:].astype('int') 
    ...
    def do_work(self, data):
        ...
        svc = SVC(kernel='rbf',gamma=ls[gamma],C=ls2[c], class_weight='balanced')
        #svc.fit(X_train,y_train)
        svc.fit(self.X[:,:int(ls3[pc])],self.y)
        #score = svc.score(self.X2[:,:int(ls3[pc])],self.y2)
        score = validateResult(svc.predict(self.X2[:,:int(ls3[pc])]), self.y2)
        
        retString = "Slave: {0} completes fold {1}, gamma {2:.4g}, C {3:.4g}, {4}pcs in {5}"
        retString = retString.format(name, 1, ls[gamma], ls2[c], int(ls3[pc]), str(datetime.now()-time))
