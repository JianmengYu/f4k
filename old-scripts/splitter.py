...
for line in sys.stdin:
	...
	if len(line) > 2:
		...
		if len(tokens) < 9: #IGNORE BROKEN LINES
			continue
		if (tokens[0] == "") or (len(tokens[0]) > 11): #FUCKTHISSHIT INCASE SHIT BROKE
			continue
		...
		if (int(tokens[0]) % 1000) == 0:
			...
			print("Buffer length: " + str(size))
			print("Probably still need " + str(hour) + " hours, " + str(minute) + " minutes")
			print("-----")
			#Cleanup buffer.
